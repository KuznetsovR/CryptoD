...
export class WalletsComponent implements OnInit {
  ...
  wallets: UserWallet[] = [];
  constructor(private bsc: BscApiService, private dappRadarService: DappRadarService, private userService: UserService) {}
  ...
  async importWallet() {
    ...
    try {
      ...
      (await this.bsc.getAccountBalance(tokensArray, walletAddress)).map((token) => {
        ...
        if (token.balance) this.userService.addTokenToWallet(walletAddress, token.token, token.balance);
      });
